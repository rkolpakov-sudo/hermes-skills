...
# --- safe auto-repair (spec §5.1) ----------------------------------------
# Only the genuinely safe, schema-correct fix is applied:

def repair_docx(path: str, out_path: str) -> dict:
    """Rewrite the package applying the safe minor fixes. Returns a report."""
    zf = zipfile.ZipFile(path)
    names = zf.namelist()
    fixed = {"xml_space_preserved": 0, "parts_touched": []}
    fixed_data = {}
    for name in names:
        if not (name.endswith(".xml") or name.endswith(".rels")):
            fixed_data[name] = zf.read(name)
            continue
        root = etree.fromstring(zf.read(name))
        touched = False
        for t in root.iter(f"{{{W}}}t"):
            text = t.text or ""
            if (text[:1].isspace() or text[-1:].isspace()) and \
               t.get(f"{{http://www.w3.org/XML/1998/namespace}}space") is None:
                t.set(f"{{http://www.w3.org/XML/1998/namespace}}space",
                      "preserve")
                fixed["xml_space_preserved"] += 1
                touched = True
        if touched:
            fixed["parts_touched"].append(name)
        fixed_data[name] = etree.tostring(root, xml_declaration=True,
...
def main() -> int:
    ...
    ap.add_argument("--repair", metavar="OUT",
                    help="write a repaired copy to OUT (safe xml:space fixes)")
    args = ap.parse_args()
    report = validate(args.path)
    print(json.dumps(report, ensure_ascii=False))
    if args.repair:
        fixed = repair_docx(args.path, args.repair)
        print(json.dumps({"repaired": True, "out": args.repair, **fixed},
                         ensure_ascii=False))
