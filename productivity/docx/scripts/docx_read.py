...
Usage:
  ...
  docx_read.py file.docx --markdown    # clean Markdown (mammoth, pandoc-free)
  docx_read.py file.docx --pdf out.pdf # render to PDF (docx2pdf + MS Word)
...
def extract_markdown(path: str) -> str:
    """Convert the document to clean Markdown via mammoth (pandoc-free)."""
    import mammoth
...
def convert_to_pdf(path: str, out_path: str) -> str:
    """Render the document to PDF via docx2pdf (uses MS Word on Windows).

    Returns the output PDF path. Raises if docx2pdf/Word is unavailable.
    """
...
def main() -> int:
    ...
    g.add_argument("--markdown", action="store_true",
                   help="convert to Markdown via mammoth (pandoc-free)")
    g.add_argument("--pdf", metavar="OUT",
                   help="render to PDF via docx2pdf (needs MS Word; Windows)")
    args = ap.parse_args()

    if args.pdf:
        try:
            out = convert_to_pdf(args.path, args.pdf)
        except Exception as exc:  # noqa: BLE001
            print(json.dumps({"ok": False, "error": str(exc),
                               "reason": "docx2pdf/Word unavailable"},
                              ensure_ascii=False), file=sys.stderr)
            return 1
        print(json.dumps({"ok": True, "output": out}, ensure_ascii=False))
        return 0
    if args.markdown:
        print(extract_markdown(args.path))
        return 0
    if args.images:
