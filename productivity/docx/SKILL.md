...
- `mammoth` (`pip install mammoth`) for `--markdown` export — pure Python,
  no pandoc/LibreOffice needed.
- `docx2pdf` (`pip install docx2pdf`) for `--pdf` export — requires MS Word
  (Windows/macOS). Renders through Word's own engine, so output is
  pixel-identical to opening the file in Word. Not available on hosts
  without Word; the script reports the error instead of crashing.
...
| Convert to Markdown (pandoc-free) | `docx_read.py f.docx --markdown` |
| Render to PDF | `docx_read.py f.docx --pdf out.pdf` |
...
2. **Read.** Use `scripts/docx_read.py` with exactly one mode flag.
   ...
   plus paragraph/table/section counts. `--markdown` converts the whole
   document to clean Markdown via `mammoth` (pure Python, no pandoc).
   `--pdf OUT` renders the document to PDF via `docx2pdf` (uses MS Word's
   own renderer — pixel-accurate, Windows/macOS only). `--images DIR`
   copies every file under `word/media/` out of the package.
...
**Lighter alternative on Windows (preferred when MS Word is present):**
`docx2pdf` uses Word's COM automation, so it needs zero extra binaries
and produces pixel-identical output to Word. Check for Word first:
`ls "/c/Program Files/Microsoft Office/root/Office16/WINWORD.EXE"`.
If present: `pip install docx2pdf && docx2pdf file.docx`. Prefer this
over installing LibreOffice purely for PDF export.
...
- **`--pdf` needs MS Word.** `docx_read.py --pdf` renders through
  `docx2pdf`, which drives a locally installed Word (COM/AppleScript).
  Windows and macOS only — on a host without Word it returns
  `{"ok": false, ...}` without crashing. The rendered PDF is pixel-identical
  to opening the file in Word, so it is the preferred path over LibreOffice
...
- **Prefer pure-Python deps over heavy native binaries.** When a
  capability can be met by a small `pip install` package, use it instead
  of shelling out to LibreOffice/pandoc/etc. This session replaced pandoc
  with `mammoth` for Markdown export. DOCX→PDF is the one case with no
  good pure-Python engine: on Windows with MS Word installed, use
  `docx2pdf` (COM automation, zero extra binaries) rather than installing
  LibreOffice; reserve LibreOffice only as a last resort. The user
  explicitly rejected installing extra programs just to improve a skill.
