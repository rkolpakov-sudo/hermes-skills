...
Cell object keys (all optional except value/formula):
  ...
  semantic       "input" | "cross_sheet" | "external" | "needs_input"
                 Applies the financial-model color coding (see FINANCIAL_MODEL
                 preset): input = blue font, cross_sheet = green font,
                 external = red font, needs_input = yellow fill. Overrides
                 explicit font_color/fill when both present.

Sheet-level "preset": "financial_model" applies the default font (Times New
Roman 11pt) and the header style (bold, light-blue fill) across the sheet;
it does NOT override per-cell style keys you set explicitly.
...
# --- Financial-model semantic color coding (Document Engine §4.2) ----
# Hardcoded inputs -> blue font, cross-sheet links -> green, external
# links -> red, cells that still need user input -> yellow fill.
SEMANTIC_FONT = {
    "input": "0000FF",
    "cross_sheet": "008000",
    "external": "FF0000",
...
FINANCIAL_MODEL_FONT = "Times New Roman"
FINANCIAL_MODEL_FONT_SIZE = 11
FINANCIAL_MODEL_HEADER_FILL = "D9E2F3"


def _apply_semantic(cell, semantic):
    if semantic in SEMANTIC_FONT:
        cell.font = Font(color=SEMANTIC_FONT[semantic],
                         size=cell.font.sz or FINANCIAL_MODEL_FONT_SIZE,
                         name=cell.font.name or FINANCIAL_MODEL_FONT,
                         bold=cell.font.b)
    elif semantic in SEMANTIC_FILL:
        cell.fill = PatternFill("solid", fgColor=SEMANTIC_FILL[semantic])
    else:
...
def apply_cell(ws, coord, spec):
    ...
    if isinstance(spec, dict):
        ...
        if spec.get("semantic"):
            _apply_semantic(cell, spec["semantic"])
...
def build_sheet(ws, spec):
    # financial_model preset: default font + bold light-blue header row
    if spec.get("preset") == "financial_model":
        ws.font = Font(name=FINANCIAL_MODEL_FONT, size=FINANCIAL_MODEL_FONT_SIZE)
        header_fill = PatternFill("solid", fgColor=FINANCIAL_MODEL_HEADER_FILL)
    for row in spec.get("rows", []):
        ...
        # apply header styling when a financial_model preset is active
        if spec.get("preset") == "financial_model" and r == 1:
            for cell in ws[r]:
                cell.font = Font(name=FINANCIAL_MODEL_FONT,
                                 size=FINANCIAL_MODEL_FONT_SIZE, bold=True)
