...
- Recalculating formulas headlessly via the pure-Python `formulas`
  engine (`xlsx_recalc.py`) — no LibreOffice needed.
...
- Python 3.10+ with `openpyxl` (`pip install openpyxl`) for create/read/edit.
- `formulas` (`pip install formulas`) — pure-Python calculation engine used
  by `xlsx_recalc.py` to materialize cached formula results headlessly.
  No LibreOffice / Excel / JVM required.
- Optional: LibreOffice (`soffice`) only for DOC→xlsx legacy conversion.
...
| Recalculate headlessly (formulas) | `xlsx_recalc.py f.xlsx` |
...
3. **Read**: `--sheets` for inventory (names, dimensions, merged
   ...
   results headlessly run `xlsx_recalc.py file.xlsx` (uses the pure-Python
   `formulas` engine — no LibreOffice; prints `{"recalculated": false, ...}`
   and exits 0 if `formulas` raises on an unsupported function), then reload
   with `--data-only`.
...
- **Prefer pure-Python deps over heavy native binaries.** Formula
  recalculation uses the `formulas` pip package instead of shelling out
  to LibreOffice `soffice` — same cached results, no 300 MB install.
  Apply this rule generally when extending the skill: reach for a small
  pure-Python wheel before a heavy native binary. The user explicitly
  rejected installing extra programs just to improve a skill.
- **`formulas` renames sheets to UPPERCASE.** When `xlsx_recalc.py`
  rewrites the workbook, sheet names like `Смета` become `СМЕТА`. The data
  and formulas are intact; only the display/case of sheet names changes. If
  you reference sheets by exact case elsewhere, account for this, or keep
  ASCII sheet names. If `formulas` raises on an Excel function it does not
  implement, recalc reports `{"recalculated": false, ...}` and leaves the
